...
- (void)viewDidLoad
{
    ...
    [self.signaling connectToServer:@"signaling.simplewebrtc.com" port:443 secure:YES success:^{
        [self.signaling joinRoom:@"wu154" success:^{
            NSLog(@"join success");
        } failure:^{
            NSLog(@"join failure");
        }];
...
- (void)configureLocalPreview {
    ...
    // There is a chance that this video source is not an RTCAVFoundationVideoSource,
    // but we know it should be from TLKWebRTC
